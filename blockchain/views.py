...
class Blockchain:
    ...
    def create_block(self, nonce, previous_hash):
        block = {'index': len(self.chain) + 1, 'timestamp': str(datetime.datetime.now()),
                 'nonce': nonce, 'previous_hash': previous_hash, 'transactions': self.transactions}
        self.transactions = []
    ...
    def proof_of_work(self, previous_hash):
        ...
        while check_nonce is False:
            hash_operation = hashlib.sha256(str(new_nonce ** 2 - previous_hash ** 2).encode()).hexdigest()
...
# creating an address for the node running on the server
...
def mine_block(request):
    if request.method == "GET":
        ...
        blockchain.add_transaction(sender=root_node,receiver=node_address, amount=7.1,
                                   time=str(datetime.datetime.now()))
...
def connect_node(request): #New
    if request.method == 'POST':
        ...
        response = {'message': 'All the nodes are now connected. The town now contains the following nodes:',
